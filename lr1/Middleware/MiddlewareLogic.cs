namespace lr_one.Middleware
{
    public class MiddlewareLogic
    {
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            if (path == "/ex1")
            {
                if (context.Request.Query.Keys.Contains("name") && context.Request.Query.Keys.Contains("year") && context.Request.Query.Keys.Contains("countries") && context.Request.Query.Keys.Contains("staff"))
                {
                    Company responseCompany = new Company();
                    responseCompany.Name = context.Request.Query["name"].ToString();
					Int32.TryParse(context.Request.Query["year"].ToString(), out int year);
					responseCompany.Year = year;
                ...
                {
                    await context.Response.WriteAsync($"Error QueryString! Try this format: \"?name=...&year=...&countries=...&staff=...\"");
                }
            }
            else if (path == "/ex2")
            {
